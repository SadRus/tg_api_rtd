...
![PyPI - Downloads](https://img.shields.io/pypi/dm/tg_bot_api)
![PyPI - License](https://img.shields.io/pypi/l/tg_bot_api)

Библиотека tg_bot_api упрощает работу с веб-API Telegram. Она предоставляет тонкую обёртку над веб API Telegram и библиотекой [HTTPX](https://www.python-httpx.org/). Библиотека tg_bot_api добавляет к HTTPX схемы данных и удобные часто используемые функции, но не мешает, при необходимости, спускаться ниже на уровень HTTP-запросов.

Ключевые возможности библиотеки tg_bot_api:
...
Библиотека tg_bot_api предлагает несколько необычных концепций для работы с API. Пробежимся по ним вкратце.
...
```py
from tg_bot_api import SyncTgClient, SendMessageRequest
...
```py
from tg_bot_api import SyncTgClient, DeleteMessageRequest
...
```py
from tg_bot_api import SyncTgClient, EditMessageTextRequest
...
```py
from tg_bot_api import SyncTgClient, EditMessageCaptionRequest
...
```py
from tg_bot_api import SyncTgClient, EditUrlMessageMediaRequest
...
```py
from tg_bot_api import SyncTgClient, EditBytesMessageMediaRequest, InputMediaBytesDocument
...
```py
from tg_bot_api import SyncTgClient, InlineKeyboardButton, InlineKeyboardMarkup
...
```py
from tg_bot_api import SyncTgClient, SendBytesPhotoRequest
...
```py
from tg_bot_api import SyncTgClient, SendUrlPhotoRequest
...
```py
from tg_bot_api import SyncTgClient, SendBytesDocumentRequest
...
```py
from tg_bot_api import SyncTgClient, SendUrlDocumentRequest
...
```py
from tg_bot_api import AsyncTgClient, SendMessageRequest
...
```py
from tg_bot_api import AsyncTgClient, DeleteMessageRequest
...
```py
from tg_bot_api import AsyncTgClient, EditMessageTextRequest
...
```py
from tg_bot_api import AsyncTgClient, EditUrlMessageMediaRequest
...
```py
from tg_bot_api import AsyncTgClient, EditBytesMessageMediaRequest, InputMediaBytesDocument
...
```py
from tg_bot_api import AsyncTgClient, InlineKeyboardButton, InlineKeyboardMarkup
...
```py
from tg_bot_api import AsyncTgClient, EditMessageCaptionRequest
...
async def main(token: str, chat_id: int, photo_filename: str) -> None:
    async with tg_bot_api.AsyncTgClient.setup(token):
        async with aiofiles.open(photo_filename, 'rb') as f:
            photo_content = await f.read()
        tg_request = tg_bot_api.SendBytesPhotoRequest(chat_id=chat_id, photo=photo_content, filename=photo_filename)
        await tg_request.asend()
```
...
async def main(token: str, chat_id: int, photo_filename: str, photo_url: str) -> None:
    async with tg_bot_api.AsyncTgClient.setup(token):
        tg_request = tg_bot_api.SendUrlPhotoRequest(chat_id=chat_id, photo=photo_url, filename=photo_filename)
        await tg_request.asend()
```
...
async def main(token: str, chat_id: int, document_filename: str) -> None:
    async with tg_bot_api.AsyncTgClient.setup(token):
        ...
        tg_request = tg_bot_api.SendBytesDocumentRequest(chat_id=chat_id, document=document_content, filename=document_filename)
        await tg_request.asend()
```
...
async def main(token: str, chat_id: int, document_filename: str, document_url: str) -> None:
    async with tg_bot_api.AsyncTgClient.setup(token):
        tg_request = tg_bot_api.SendUrlDocumentRequest(chat_id=chat_id, document=document_url, filename=document_filename)
        await tg_request.asend()
```
...
в библиотеку tg_bot_api. Можно добавлять свои типы запросов и ответов API, менять способ отправки HTTP-запросов и реакции на ответ.

Пример использования низкоуровневого асинхронного API:

```py
from httpx import Response as HttpResponse
from tg_bot_api import AsyncTgClient, SendMessageRequest, SendMessageResponse, raise_for_tg_response_status
...
- [tg_types.py](./tg_methods.py) -- библиотека типов данных, с которыми работает Telegram Bot API
...
Вот пример как добавить в зависимости библиотеку asks. Запустите все контейнеры. Подключитесь к уже работающему контейнеру `tg-bot-api` и внутри запустите команду `poetry add asks`. Затем выйдите из контейнера и остановить работу контейнеров:

```shell
$ docker compose up -d
$ docker compose exec tg-bot-api bash
...
```shell
$ docker compose build tg-bot-api
```
...
Линтеры запускаются в отдельном docker-контейнере, а код подключается к нему с помощью volume. Например, чтобы проверить линтером код в каталогах `tg_bot_api` и `tests` запустите команду:

```shell
$ docker compose run --rm py-linters flake8 /tg_bot_api/ /tests/
[+] Building 0.0s (0/0)
[+] Building 0.0s (0/0)
/tg_bot_api/client.py:23:121: E501 line too long (148 > 120 characters)
1
```
...
```shell
$ docker compose run --rm tg-bot-api pytest
=========================== test session starts ===========================
...
```shell
$ docker compose run --rm tg-bot-api pytest -s test_asend.py::test_httpx_mocking
```
